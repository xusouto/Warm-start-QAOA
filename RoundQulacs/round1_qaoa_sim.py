...
# Qulacs Utils
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def get_expval(n, candidate_circuit, hamiltonian):
    state = QuantumState(n)
    sim = QuantumCircuitSimulator(candidate_circuit, state)
    #sim.initialize_state(0)
    sim.simulate()
    return sim.get_expectation_value(hamiltonian)
# -------------------------------------------------------------------------------------------------------
def cost_func_estimator(params, n, hamiltonian, pauli_params, pauli_weights, warm: bool, lukewarm : bool, pre_sol , eps, depth):
    ...
    e = get_expval(n, ansatz, hamiltonian)
    return e
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

# Parameter initialization w/ grid search
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def fit_ab_from_two_points(beta_vals, energy_vals):
    """
    Solve [sin(2β) sin(4β)] [a b]^T = E  (2 equations, 2 unknowns).
    Works as an exact solve (or least-squares if you pass >2 points).
    """
...
def ising_from_terms(
    ...
    Build:
      ...
      - (optionally) a Qulacs Observable using the same coefficients.

    Returns:
      SparsePauliOp, Observable | None, offset
    """
    ...
    labels = []
    # we also keep the boolean masks so we can map to Qulacs indices cleanly
    masks = []
    ...
    # ---- Qulacs Observable (same coefficients, no constant term) ----
    qulacs_obs: Optional[Observable] = None
    if build_qulacs_observable:
        qulacs_obs = Observable(n)
        # Qulacs expects strings like "Z 0 Z 3" (no explicit I's)
        for mask, coeff in zip(masks, w):
            ...
            # constant part (offset) is *not* added here; it is kept separately

    # we keep offset separate for simulation/energy-shift purposes
    return qulacs_obs
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------------------------------
n=30                                                                    # Size of graph
eps = np.linspace(0.25,0.25,1)                                          # Reg. parameter
energies = []
# -------------------------------------------------------------------------------------------------------
