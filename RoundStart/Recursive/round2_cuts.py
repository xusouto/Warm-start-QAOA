...
iteration = args.iter
path = args.folder
with open(os.path.join(path, "Graphs", f"graph{n_}_{flag}_{iteration}.json"), "r", encoding="utf-8") as f:
    data = json.load(f)   
...
cuts = extract_gw_cuts(sol, top_k=10, best_m=5)
cuts_list = [d["x"] for d in cuts]
N_CUTS=len(cuts)
# -------------------------------------------------------------------------------------------------------
# Outputs saved in .json files
# -------------------------------------------------------------------------------------------------------
out = {"cuts": cuts_list, "numcuts": N_CUTS}
out_dir = Path(path) / "Cuts"
out_dir.mkdir(parents=True, exist_ok=True)
out_path = out_dir / f"cuts{n_}_{flag}_{iteration}.json"
